...
\begin{frame}{Example II - ii}
  \begin{figure}
    \begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
      ...
      \node[state,accepting] (p_12) [right=of p_3] {\small$p_1, p_2$};
      \node[state,accepting] (p_23) [right=of p_12] {\small$p_2, p_3$};
      ...
      \path[dashed]
        (p_1) edge node {1} (q_1)
        (p_2) edge node {2} (q_23)
        (p_3) edge node {3} (q_12);
      \path[dashed]
        (p_12) edge node {4} (q_123)
        (p_23) edge node {5} (q_123)
        (p_123) edge node {6} (q_123);
      \path[->]
        (p_1) edge node {a} (p_2)
        (p_2) edge node {a} (p_3)
        (p_3) edge node {a} (p_12)
        (p_12) edge node {a} (p_23)
